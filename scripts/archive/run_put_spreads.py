...
def main() -> None:
    ...
    parser.add_argument(
        "--ma-filter", type=int, default=None, metavar="N",
        help="Show MA trend filter comparison: how results change when requiring "
             "spot > N-day MA on entry (e.g. --ma-filter 50). Requires --detail-short-delta "
             "and --detail-wing. Does not suppress any entries from the main sweep.",
    )
    parser.add_argument(
