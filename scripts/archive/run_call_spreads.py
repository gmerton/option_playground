#!/usr/bin/env python3
"""
Generic bear call spread backtest — short delta × wing width × VIX regime sweep.
...
# Default run for a given ticker:
  PYTHONPATH=src python run_call_spreads.py --ticker UVXY
  PYTHONPATH=src python run_call_spreads.py --ticker TLT

# With a 25% spread filter on the short leg:
  PYTHONPATH=src python run_call_spreads.py --ticker TLT --spread 0.25
...
# Custom parameters:
  PYTHONPATH=src python run_call_spreads.py --ticker UVXY \\
      --short-deltas 0.40,0.50 --wing-widths 0.10,0.20 --dte 30
...
from lib.studies.call_spread_study import run_call_spread_study
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generic bear call spread backtest — short delta × wing width × VIX filter",
    ...
    parser.add_argument(
        ...
        help="Comma-separated unsigned short call delta targets, e.g. 0.30,0.40,0.50; "
    ...
    output_csv     = (
        None if args.no_csv
        else (args.output or f"{ticker_lower}_call_spreads_{date.today().isoformat()}.csv")
    )

    run_call_spread_study(
        ticker=args.ticker,
        start=start,
        end=args.end,
