...
| **Debit call spread as a vehicle** — extend the August vehicle study (calls / short puts / put spreads) with a 30-delta 10-wide bull call spread on identical entries | whether capping the upside changes the vehicle ranking, or whether it follows entry quality like everything else | QUEUED 2026-09-20 (Ravish super-bull-call review), `data/options_with_ravish/super_bull_call_spread_2026-09_review.md` | ½ day |
| **Does capping hurt the event trade** — re-run the event-convexity winner (0.12Δ call, sell ≤5d, +30.7%) with the call replaced by a 30-delta debit spread | whether the one validated call-buying edge survives a short leg; it is a right-tail result, so the prior is no | QUEUED 2026-09-20 (same review), memory `project_catalyst_studies` | ½ day |
