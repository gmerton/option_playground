...
Five components, 100 points, each tied to a rule already on the plan sheet:
  ...
  same-day trips  25  stock same-day round trips: <=3 full, 4-6 -> 15, 7-10 -> 5, >10 -> 0
...
def main() -> int:
    ...
    conn = _get_conn()
    t = pd.read_sql("SELECT trade_date, conid, underlying_symbol, asset_category, open_close, realized_pnl FROM journal_trades", conn)
    ...
    for d in sorted(t.trade_date.unique()):
        ...
        s_rt = 25 if stk_rt <= 3 else 15 if stk_rt <= 6 else 5 if stk_rt <= 10 else 0
        ...
        rows.append(dict(date=d, fills=len(x), symbols=x.underlying_symbol.nunique(), stock_round_trips=stk_rt, doubled=doubled,
    ...
    for r in df.itertuples():
        lines.append(f"| {r.date} | **{r.grade}** | {r.score} | {r.fills} | {r.symbols} | {r.stock_round_trips} | {r.doubled} | {r.disc_good}/{r.disc_bad} of {r.disc_entries} | {r.too_soon} | {'' if pd.isna(r.nav_pnl) else f'{r.nav_pnl:+,.0f}'} |")
