...
import pandas as pd
from lib.option_strat import query_entries_range_for_strategy, summarize_hold_to_maturity_strategy_from_entries, summarize_strangle_trades
from lib.athena_lib import fetch_strangle_trades
from lib.mysql_lib import upsert_strangle_detail, upsert_strangle_summary
...
def strangle_study(tickers):
    ...
    for i in range(0, len(tickers), _STRANGLE_BATCH_SIZE):
        ...
        print(f"\nFetching batch {batch_num}/{n_batches}: {batch}")
    ...
    print(f"\n[TIMING] fetch: {t1-t0:.2f}s  ({len(df_all)} rows, {len(tickers)} ticker(s))")
    ...
    detail_affected  = upsert_strangle_detail(detail_all)
    summary_affected = upsert_strangle_summary(summaries_mid, summaries_worst)

    t2 = time.perf_counter()
    print(f"\n[TIMING] total: {t2-t0:.2f}s")
    print(f"Saved: {output_path}")
    print(f"Saved: {detail_path}")
    print(f"MySQL: {detail_affected} rows upserted into strangle_study_detail")
    print(f"MySQL: {summary_affected} rows upserted into strangle_study_summary")
