"""
Bear call spread backtest engine — short delta × wing width × VIX regime sweep.

Strategy
--------
- Sell a call at short_delta_target (Friday, target DTE)
- Buy a call at (short_delta_target - wing_delta_width)  ← further OTM, higher strike
...
Sweep dimensions
----------------
  short_delta_targets  e.g. [0.30, 0.35, 0.40, 0.50]
  wing_delta_widths    e.g. [0.10, 0.15, 0.20]
  vix_thresholds       e.g. [None, 30, 25, 20]   (None = no filter / baseline)

Capital / Margin
----------------
Max loss: (spread_width - net_credit_mid) × 100
  where spread_width = long_strike - short_strike (in dollars)
...
def build_call_spread_trades(
    ...
    """
    Find bear call spread entries from the options cache.

    short_delta_target: unsigned call delta for the short leg (e.g. 0.40)
    wing_delta_width:   delta separation between legs (e.g. 0.15)
                        → long call target delta = short_delta_target - wing_delta_width

    max_spread_pct: bid-ask filter applied to the short leg (ask-bid)/mid ≤ threshold.
    max_delta_err:  used for both legs independently.

    Long strike > short strike (bear call spread = credit received).
    ...
    # Base filter: calls on entry day within DTE range with valid quotes
    mask = (
    ...
    if max_spread_pct is not None:
        ...
        short_pool = calls[calls["_spread_pct"] <= max_spread_pct].copy()
    else:
        short_pool = calls.copy()
    ...
    # Bear call spread: long_strike > short_strike (long call is further OTM)
    spreads = spreads[spreads["long_strike"] > spreads["short_strike"]].copy()
    if spreads.empty:
        return pd.DataFrame()
    ...
    # Spread geometry
    spreads["spread_width"]        = spreads["long_strike"] - spreads["short_strike"]
...
def find_spread_exits(
    ...
    e.g. stop_multiple=2 exits when it costs 2× the premium collected to close
    (i.e. you've lost 1× your credit).
    ...
    # All call daily marks
    call_marks = (
        df_opts[df_opts["cp"] == "C"]
    ...
    relevant = call_marks.merge(all_keys, on=["expiry", "strike"], how="inner")
...
def run_spread_delta_sweep(
    ...
    for short_delta in short_delta_targets:
        for wing_width in wing_delta_widths:
            ...
            positions = find_spread_exits(positions, df_opts, profit_take_pct=profit_take_pct, ann_target=ann_target)
...
def print_spread_sweep_summary(
    ...
    for wing_width in wing_delta_widths:
        print(f"\n{bar}")
        print(
            f"  {ticker} Bear Call Spread — {dte_target} DTE  "
